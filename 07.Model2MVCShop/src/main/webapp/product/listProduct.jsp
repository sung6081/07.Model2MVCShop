...
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top:10px;">
	...
	--%>
	<f:set var="i" value="${searchVO.pageSize }" />
	...
	<tr class="ct_list_pop">
		...
		<td></td>
			<f:if test="${empty product.proTranCode }">
				<td align="left"><a href="/product/getProduct.do?prodNo=${product.prodNo }<%-- productVO.getProdNo() --%>&menu=${param.menu }<%-- menu --%>">${product.prodName }<%-- productVO.getProdName() --%></a></td>
			</f:if>
			<f:if test="${!empty product.proTranCode }">
				<td align="left">${product.prodName }<%-- productVO.getProdName() --%></td>
			</f:if>
		<td></td>
		<td align="center"><img src = "/images/uploadFiles/${product.fileName }" width="50" height="50"/></td>
		<td></td>
		<td align="left">${product.price }<%-- productVO.getPrice() --%></td>
		<td></td>
		<td align="left">${product.regDate }<%-- productVO.getRegDate() --%></td>
		<td></td>
		<td align="left">
		...
		<f:if test="${product.proTranCode eq '0' }">
			...
			<f:if test="${param.menu eq 'manage' }">
					<a href="/purchase/updateTranCodeByProd.do?prodNo=${product.prodNo }&tranCode=1">배송하기</a>
			</f:if>
			<%-- } --%>
		</f:if>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top:10px;">
	<tr>
		...
		<%--
			...
			<!-- <a href="/product/listProduct.do?page=${resultPage.beginUnitPage - 1 }&menu=${param.menu }&searchCondition=${searchVO.searchCondition }&searchKeyword=${searchVO.searchKeyword }">◀ 이전</a> -->
			</f:if>
		<f:if test="${ empty searchVO.searchCondition || empty searchVO.searchKeyword }">
			<a href="javascript:fncGetProductList(${resultPage.beginUnitPage - 1 })">◀ 이전</a>
			<!-- <a href="/product/listProduct.do?page=${resultPage.beginUnitPage - 1}&menu=${param.menu }">◀ 이전</a> -->
		</f:if>		
		...
		<f:when test="${!empty searchVO.searchCondition && !empty searchVO.searchKeyword }">
			<a href="javascript:fncGetProductList(${i})">${i }<%-- i --%></a> 
			<!-- <a href="/product/listProduct.do?page=${i }<%--i --%>&menu=${param.menu }<%--menu--%>&searchCondition=${searchVO.searchCondition }<%--searchVO.getSearchCondition() --%>&searchKeyword=${searchVO.searchKeyword }<%-- searchVO.getSearchKeyword() --%>">${i }<%-- i --%></a> -->
		</f:when>
		
		<f:otherwise>
			<a href="javascript:fncGetProductList(${i})">${i }<%-- i --%></a> 
			<!-- <a href="/product/listProduct.do?page=${i }<%-- i --%>&menu=${param.menu }<%-- menu --%>">${i }<%-- i --%></a> -->
		</f:otherwise>
		</f:choose>
		</f:forEach>
		...
		<f:choose>
		<f:when test="${!empty searchVO.searchCondition && !empty searchVO.searchKeyword }">
			<a href="/product/listProduct.do?page=${resultPage.endUnitPage + 1 }{<%-- right + 1 --%>&menu=${param.menu }<%-- menu --%>&searchCondition=${searchVO.searchCondition }<%-- searchVO.getSearchCondition() --%>&searchKeyword=${searchVO.searchKeyword }<%-- searchVO.getSearchKeyword() --%>">이후 ▶</a>
		</f:when>
		<%--	}else { --%>
		<f:otherwise>
			<a href="/product/listProduct.do?page=${resultPage.endUnitPage + 1 }<%-- right + 1 --%>&menu=${param.menu }<%-- menu --%>">이후 ▶</a>
		</f:otherwise>
			</f:choose>
			</f:otherwise>
			</f:choose>
